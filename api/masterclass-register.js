...
const MASTERCLASS_GROUP_ID = process.env.MASTERCLASS_GROUP_ID || '192868905458336945';
...
module.exports = async function handler(request, response) {
  ...
  if (!apiKey || !MASTERCLASS_GROUP_ID) {
    return json(response, 500, {
      ok: false,
      message: 'Masterclass registration is not configured yet.'
    });
  }
  ...
  try {
    ...
    if (!mailerLiteResponse.ok) {
      return json(response, 502, {
        ok: false,
        message: 'MailerLite could not accept the registration right now.'
      });
    }
